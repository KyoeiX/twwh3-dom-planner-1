#!/usr/bin/env python3
"""
Download TWWStats unit-card images and write a frontend-safe manifest.

Use this for any faction. It does not edit the frontend.

Examples:
  python tools/download_tww_unit_card_images.py --input norsca_twwstats_roster.json --faction norsca --test-count 3
  python tools/download_tww_unit_card_images.py --input norsca_twwstats_roster.json --faction norsca
  python tools/download_tww_unit_card_images.py --input khorne_twwstats_roster.json --faction khorne
...
UA = "Mozilla/5.0 TWWUnitCardDownloader/1.0"
...
def detect_faction(path: Path, units: List[Dict[str, Any]], override: str = "") -> str:
    ...
    stem = slug(path.stem)
    for cut in ("_twwstats", "_roster", "_unit", "_export", "_images"):
        if cut in stem:
...
def unit_name(unit: Dict[str, Any], index: int) -> str:
    ...
    return str(unit.get("n") or unit.get("name") or land.get("onscreen_name") or unit_id(unit, index))


def variant_code(unit: Dict[str, Any]) -> str:
    land = unit.get("land_unit") or {}
    variant = land.get("variant") or {}
    return str(variant.get("unit_card_url") or unit.get("unit_card_url") or "")
...
def image_candidates(unit: Dict[str, Any]) -> List[str]:
    ...
    card = variant_code(unit)
    if card and card not in PLACEHOLDERS:
        add_unique(out, f"ui/units/icons/{card}.webp")
        add_unique(out, f"ui/units/icons/{card}.png")
        add_unique(out, f"ui/portraits/units/no_culture/{card}.webp")
        add_unique(out, f"ui/portraits/units/no_culture/{card}.png")
...
def main() -> int:
    ...
    parser.add_argument("--input", "-i", required=True, help="Raw TWWStats roster JSON")
    ...
    tww_version, units = get_units(doc)
    if args.limit:
    ...
    for index, unit in enumerate(units, 1):
        ...
        test = bool(args.test_count and index <= args.test_count)
        if test:
            print(f"\nTEST {index}")
